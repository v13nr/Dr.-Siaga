...
{
	/*
	*	$tgl_lahir = tgl kelahiran dgn format 'Y-m-d'
	*/
...
if ( ! function_exists('con_hari_eng_to_indo')){

	/* konversi hari dari format english ke format indo */
	
	function con_hari_eng_to_indo($tgl){
...
if (!function_exists('tambah_tgl')) 
{
	function tambah_tgl($tgl,$days){
		$res = date('Y-m-d H:i:s', strtotime($days.' days', strtotime($tgl)));
...
/*	
* 	function unttuk memanipulasi tanggal 
*	penjumlahan / pengurangan tanggal/bulan/tahun
*	pengurangan di berikan tanda minus
*	param 	$tgl = tanggal
*			$d 	 = days
*			$m 	 = month
*			$y	 = year
...
if (!function_exists('manipulasi_tgl')) 
{
	function manipulasi_tgl($tgl,$d,$m,$y){
		$res = date('Y-m-d', strtotime($d.' days '.$m.' month '.$y.' year', strtotime($tgl)));
