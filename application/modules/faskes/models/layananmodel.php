...
class LayananModel extends CI_Model{
	...
	public  function getlist($start,$finish,$search){ 
		...
		$this->dbmaster->order_by('a.nama_layanan','asc');
	...
	public function hapus($id=0){
		$this->dbmaster->where('layanan_id',$id);
	...
	public function get_by_id($id){ 
		...
		$this->dbmaster->where('a.layanan_id', $id);
	...
	public function saveData($data)
	{
		...
		if(empty($data['layanan_id'])){
			...
			$this->dbmaster->where('layanan_id',$data['layanan_id']);
