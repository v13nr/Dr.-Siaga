...
class Invoice extends CI_Controller {
	...
	public function index($page=0){
		...
		$config['total_rows'] 	= $this->InvoiceModel->jmlhdata($filter);
		$config['per_page'] 	=  20;
		$config['first_link'] 	= '<<';
		$config['last_link'] 	= '>>';
		...
		$customerpaket = $this->InvoiceModel->getlist($config['per_page'],$page,$filter);
		
		// prepare data
		$data['jmlh_data']	= $this->InvoiceModel->jmlhdata($filter);
		$data['jmlh_item']	= $this->config->item('jmlh_item');
		$data['action'] 	= site_url().'/paket/customerpaket/add';
		$data['results'] 	= $customerpaket;
		$data['file'] 		= 'finance/invoice/list';
		$data['title'] 		= $this->config->item('app_name');
		$data['version'] 	= $this->config->item('version');
		$data['page'] 		= $page;
		$data['c'] 			= $this->c;
		$data['subtitle'] 	= "Daftar Invoice Customer ";
		$data['keyword'] 	= null;
		$data['desc'] 		= null;
