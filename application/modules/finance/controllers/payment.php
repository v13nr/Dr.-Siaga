...
class Payment extends CI_Controller {
	...
	public function index($page=0){
		...
		// load model
		$this->load->model('finance/PaymentModel');
		...
		$config['base_url'] 	= site_url()."finance/payment";
		$config['total_rows'] 	= $this->PaymentModel->jmlhdata($filter);
		$config['per_page'] 	=  20;
		$config['first_link'] 	= '<<';
		$config['last_link'] 	= '>>';
		...
		$customerpaket = $this->PaymentModel->getlist($config['per_page'],$page,$filter);
		
		// prepare data
		$data['jmlh_data']	= $this->PaymentModel->jmlhdata($filter);
		$data['jmlh_item']	= $this->config->item('jmlh_item');
		$data['action'] 	= site_url().'/finance/payment/add';
		$data['results'] 	= $customerpaket;
		$data['file'] 		= 'finance/payment/list';
		$data['title'] 		= $this->config->item('app_name');
		$data['version'] 	= $this->config->item('version');
		$data['page'] 		= $page;
		$data['c'] 			= $this->c;
		$data['subtitle'] 	= "Daftar Pembayaran Customer ";
		$data['keyword'] 	= null;
		$data['desc'] 		= null;
