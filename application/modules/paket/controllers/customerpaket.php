 ...
 *  Controller			: CustomerPaket 
**/

if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class CustomerPaket extends CI_Controller {
	private $c = 'customerpaket';
	...
	public function index($page=0){
		...
		// load model
		$this->load->model('CustomerPaketModel');
		...
		$config['base_url'] 	= site_url()."paket/customerpaket";
		$config['total_rows'] 	= $this->CustomerPaketModel->jmlhdata($filter);
		$config['per_page'] 	=  20;
		$config['first_link'] 	= '<<';
		$config['last_link'] 	= '>>';
		...
		$customerpaket = $this->CustomerPaketModel->getlist($config['per_page'],$page,$filter);
		
		// prepare data
		$data['jmlh_data']	= $this->CustomerPaketModel->jmlhdata($filter);
		$data['jmlh_item']	= $this->config->item('jmlh_item');
		$data['action'] 	= site_url().'/paket/customerpaket/add';
		$data['results'] 	= $customerpaket;
		$data['file'] 		= 'customer/list';
		$data['title'] 		= $this->config->item('app_name');
		$data['version'] 	= $this->config->item('version');
		$data['page'] 		= $page;
		$data['c'] 			= $this->c;
		$data['subtitle'] 	= "Daftar Paket Customer ";
		$data['keyword'] 	= null;
		$data['desc'] 		= null;
